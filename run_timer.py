...
def main_function():
    ...
    my_record = rubik_library.get_record(cubo)
    min, sec = rubik_library.convert_seconds(my_record)
    print(f"Record da battere {min} minuti e {sec} secondi")

    tempo_impiegato, cubo = rubik_library.calcolo_tempo(cubo)
    rubik_library.is_new_record(tempo_impiegato, cubo)
    rubik_library.diffs_media_tempo_attuale(cubo=cubo, tempo_impiegato=tempo_impiegato)
    rubik_library.diffs_record_tempo_attuale(record=my_record, tempo_impiegato=tempo_impiegato)
